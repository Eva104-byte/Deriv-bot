...
A headless 24/7 Node.js trading bot for Deriv's **Digit Differs** and **Even/Odd** markets, ready to deploy on [Railway](https://railway.app).
...
| `EO_ENABLED` | `true` to enable Even/Odd bot | Optional |
...
[12:00:17] ✅  WIN — +$0.95 | Contract 12345678
```
...
| `DIFFERS_STAKE` | `1.00` | USD stake per trade |
...
| `EO_ENABLED` | `false` | Enable Even/Odd bot |
| `EO_STAKE` | `1.00` | USD stake per E/O trade |
| `EO_TARGET_PROFIT` | `10.00` | E/O profit target |
| `EO_STOP_LOSS` | `5.00` | E/O stop loss |
...
Automated trading carries significant financial risk. Always test with a **Demo account** first. Set conservative `DIFFERS_STOP_LOSS` and `DIFFERS_TARGET_PROFIT` values.
