...
function placeDiffersTrade() {
  ...
  log(`Placing Differs: digit ${selectedDigit} | $${CONFIG.DIFFERS.STAKE.toFixed(2)} | ${CONFIG.DIFFERS.DURATION} tick(s)`, 'trade');
  ws.send(JSON.stringify({
    req_id: reqId,
    buy: 1,
    price: CONFIG.DIFFERS.STAKE,
    parameters: {
      amount:        CONFIG.DIFFERS.STAKE,
      basis:         'stake',
...
function placeEoTrade() {
  ...
  log(`[E/O] Placing ${label}: $${CONFIG.EO.STAKE.toFixed(2)} | ${CONFIG.EO.DURATION} tick(s)`, 'trade');
  ws.send(JSON.stringify({
    req_id: reqId,
    buy: 1,
    price: CONFIG.EO.STAKE,
    parameters: {
      amount:        CONFIG.EO.STAKE,
      basis:         'stake',
