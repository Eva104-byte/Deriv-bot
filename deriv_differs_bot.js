 ...
 *    • 11x Martingale     — applies 11× stake once after a loss, then resets
 ...
 *    • Risk Controls       — Target Profit & Stop Loss for the Differs bot
...
//    MARTINGALE_ENABLED       true | false               (default: true)
...
const CONFIG = {
  ...
  // ── Martingale ────────────────────────────────────────────
  // Applies 11× the base stake for ONE trade after a loss, then resets.
  MARTINGALE: {
    ENABLED:    envBool('MARTINGALE_ENABLED', true),
    MULTIPLIER: 11,
  },
...
// Martingale
let martingaleActive  = false;  // true for exactly one trade after a loss
...
function placeDiffersTrade() {
  ...
  // Determine stake: apply 11× once if martingale is armed
  let stake = CONFIG.DIFFERS.STAKE;
  if (CONFIG.MARTINGALE.ENABLED && martingaleActive) {
    stake = CONFIG.DIFFERS.STAKE * CONFIG.MARTINGALE.MULTIPLIER;
    martingaleActive = false;  // consumed — resets after this one trade
    log(`🔁 Martingale active: stake elevated to $${stake.toFixed(2)} (${CONFIG.MARTINGALE.MULTIPLIER}×)`, 'trade');
  }

  log(`Placing Differs: digit ${selectedDigit} | $${stake.toFixed(2)} | ${CONFIG.DIFFERS.DURATION} tick(s)`, 'trade');
  ...
  ws.on('message', raw => {
    ...
    if (msg.msg_type === 'proposal_open_contract') {
      ...
      if (profit >= 0) {
        ...
        // Winning trade: martingale already reset on placement; nothing extra needed
      } else {
        losses++;
        log(`LOSS — $${profit.toFixed(2)} | Contract ${poc.contract_id}`, 'loss');
        // Arm martingale for next trade (fires once)
        if (CONFIG.MARTINGALE.ENABLED) {
          martingaleActive = true;
          log(`🔁 Martingale armed — next trade will use $${(CONFIG.DIFFERS.STAKE * CONFIG.MARTINGALE.MULTIPLIER).toFixed(2)} (${CONFIG.MARTINGALE.MULTIPLIER}× stake)`, 'warn');
        }
...
log(`Martingale:    ${CONFIG.MARTINGALE.ENABLED ? `Enabled | ${CONFIG.MARTINGALE.MULTIPLIER}× stake on next trade after a loss` : 'Disabled'}`);
