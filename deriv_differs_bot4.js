...
const CONFIG = {
  ...
  DIFFERS: {
    ...
    // ── Martingale ──────────────────────────────────────────
    //   MARTINGALE_ENABLED  true | false   — double stake after each loss
    //   MARTINGALE_MULTIPLIER               — stake multiplier on loss (default 2)
    //   MARTINGALE_MAX_STEPS                — max consecutive doublings before reset
    MARTINGALE_ENABLED:    envBool ('DIFFERS_MARTINGALE_ENABLED',    false),
    MARTINGALE_MULTIPLIER: envFloat('DIFFERS_MARTINGALE_MULTIPLIER', 2.0),
    MARTINGALE_MAX_STEPS:  envInt  ('DIFFERS_MARTINGALE_MAX_STEPS',  4),
  },
  ...
  EO: {
    ...
    // ── Martingale ──────────────────────────────────────────
    MARTINGALE_ENABLED:    envBool ('EO_MARTINGALE_ENABLED',    false),
    MARTINGALE_MULTIPLIER: envFloat('EO_MARTINGALE_MULTIPLIER', 2.0),
    MARTINGALE_MAX_STEPS:  envInt  ('EO_MARTINGALE_MAX_STEPS',  4),
  },
...
// Differs Martingale state
let differsCurrentStake   = CONFIG.DIFFERS.STAKE;  // active stake (may be doubled)
let differsMartingaleStep = 0;                      // consecutive loss count
...
// E/O Martingale state
let eoCurrentStake    = CONFIG.EO.STAKE;            // active stake (may be doubled)
let eoMartingaleStep  = 0;                          // consecutive loss count
...
/** Call after a Differs WIN — resets stake to base. */
function differsMartingaleWin() {
  if (!CONFIG.DIFFERS.MARTINGALE_ENABLED) return;
  if (differsMartingaleStep > 0) {
    log(`[Martingale] Differs WIN — resetting stake from $${differsCurrentStake.toFixed(2)} → $${CONFIG.DIFFERS.STAKE.toFixed(2)}`, 'win');
  }
  differsCurrentStake   = CONFIG.DIFFERS.STAKE;
  differsMartingaleStep = 0;
}

/** Call after a Differs LOSS — doubles stake up to MAX_STEPS, then resets. */
function differsMartingaleLoss() {
  if (!CONFIG.DIFFERS.MARTINGALE_ENABLED) return;
  if (differsMartingaleStep >= CONFIG.DIFFERS.MARTINGALE_MAX_STEPS) {
    log(`[Martingale] Differs max steps (${CONFIG.DIFFERS.MARTINGALE_MAX_STEPS}) reached — resetting stake to $${CONFIG.DIFFERS.STAKE.toFixed(2)}`, 'warn');
    differsCurrentStake   = CONFIG.DIFFERS.STAKE;
    differsMartingaleStep = 0;
  } else {
    differsMartingaleStep++;
    differsCurrentStake = parseFloat(
      (CONFIG.DIFFERS.STAKE * Math.pow(CONFIG.DIFFERS.MARTINGALE_MULTIPLIER, differsMartingaleStep)).toFixed(2)
    );
    log(`[Martingale] Differs LOSS — step ${differsMartingaleStep}/${CONFIG.DIFFERS.MARTINGALE_MAX_STEPS} | next stake: $${differsCurrentStake.toFixed(2)}`, 'warn');
  }
}

/** Call after an E/O WIN — resets stake to base. */
function eoMartingaleWin() {
  if (!CONFIG.EO.MARTINGALE_ENABLED) return;
  if (eoMartingaleStep > 0) {
    log(`[Martingale] E/O WIN — resetting stake from $${eoCurrentStake.toFixed(2)} → $${CONFIG.EO.STAKE.toFixed(2)}`, 'win');
  }
  eoCurrentStake   = CONFIG.EO.STAKE;
  eoMartingaleStep = 0;
}

/** Call after an E/O LOSS — doubles stake up to MAX_STEPS, then resets. */
function eoMartingaleLoss() {
  if (!CONFIG.EO.MARTINGALE_ENABLED) return;
  if (eoMartingaleStep >= CONFIG.EO.MARTINGALE_MAX_STEPS) {
    log(`[Martingale] E/O max steps (${CONFIG.EO.MARTINGALE_MAX_STEPS}) reached — resetting stake to $${CONFIG.EO.STAKE.toFixed(2)}`, 'warn');
    eoCurrentStake   = CONFIG.EO.STAKE;
    eoMartingaleStep = 0;
  } else {
    eoMartingaleStep++;
    eoCurrentStake = parseFloat(
      (CONFIG.EO.STAKE * Math.pow(CONFIG.EO.MARTINGALE_MULTIPLIER, eoMartingaleStep)).toFixed(2)
    );
    log(`[Martingale] E/O LOSS — step ${eoMartingaleStep}/${CONFIG.EO.MARTINGALE_MAX_STEPS} | next stake: $${eoCurrentStake.toFixed(2)}`, 'warn');
  }
}
...
log(`Differs MG:    ${CONFIG.DIFFERS.MARTINGALE_ENABLED ? `Enabled | x${CONFIG.DIFFERS.MARTINGALE_MULTIPLIER} multiplier | max ${CONFIG.DIFFERS.MARTINGALE_MAX_STEPS} steps` : 'Disabled'}`);
log(`E/O Bot:       ${CONFIG.EO.ENABLED      ? `Enabled | Stake $${CONFIG.EO.STAKE}`                                      : 'Disabled'}`);
log(`E/O MG:        ${CONFIG.EO.MARTINGALE_ENABLED ? `Enabled | x${CONFIG.EO.MARTINGALE_MULTIPLIER} multiplier | max ${CONFIG.EO.MARTINGALE_MAX_STEPS} steps` : 'Disabled'}`);
